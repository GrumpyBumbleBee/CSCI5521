...
class Decision_tree:
    ...
    def generate_tree(self,data,label):
        ...
        if node_entropy < self.min_entropy:
            ...
            cur_node.label = labels[np.argmax(counts)]
            return cur_node
    ...
    def combined_metric(self,left_y,right_y):
        ...
        left_entropy = self.metric(left_y)
        right_entropy = self.metric(right_y)

        # compute weights
        total_sample = len(left_y) + len(right_y)
        left_weight = len(left_y) / total_sample
        right_weight = len(right_y) / total_sample


        result = 0
        # -------------- Add your line here -------------------------------
        # !!! use self.metric(label) to compute the entropy/gini_index
        result = (left_weight * left_entropy) + (right_weight * right_entropy)
